...
export const AboutUs : React.FC<AboutUsProps> = ({image,color,title,content}) =>{
    ...
    <div className="bg-cover bg-no-repeat w-screen h-screen" style={{ backgroundImage: url }}></div>
    <div className={`${colorDiv} rounded-lg`}> 
      <h1 className="text-5xl font-roboto text-white mb-4">{title}</h1> 
      {content.map((paragraph, index) => (
        <p key={index} className="text-2xl font-montserrat text-base text-white">
          {paragraph}
        </p>
      ))}
    </div>
